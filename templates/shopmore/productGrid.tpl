			...
			<div class="col-12 col-md-6 col-lg-4 col-xl-3">
				<div class="panel product-card d-flex flex-column">
					...
					<a class="product-image-wrapper d-flex align-items-center justify-content-center p-3" href="{$p.url}" title="{$p.product_name|escape}">
						{if $p@first}
						<img src="{$p.image_url}" class="product-image img-fluid" fetchpriority="high" decoding="async" style="max-height: 100%; object-fit: contain;" alt="{$p.product_name|escape}">
						{else}
					...
					<div class="product-info d-flex flex-column flex-grow-1">
						...
						{if $p.in_stock}
							...
							<a href="{$p.url}" class="btn btn-details w-100">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M7 15h2"/><path d="M11 11h2"/><path d="M15 7h2"/></svg>
								{'Enter width and length'|translate}
							</a>
							{else}
