{if $googleLoginUrl}
<div class="auth-social{if $googleLoginCompact} auth-social--compact{/if}">
	{if !$googleLoginCompact}
	<div class="auth-social__divider"><span>{'or continue with'|translate}</span></div>
	{/if}
	<a href="{$googleLoginUrl|escape}" class="auth-social__btn auth-social__btn--google">
		...
		<span>{'Sign in with Google'|translate}</span>
	</a>
</div>
{/if}
