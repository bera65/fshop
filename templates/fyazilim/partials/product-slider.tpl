	...
	<div class="fy-slider__track" data-fy-track>
		...
		<div class="fy-slider__item">
			<article class="fy-card{if $cardStyle == 'theme'} fy-card--theme{/if}">
				...
				{elseif $p@index < 2}
					<span class="fy-card__badge">{'New'|translate}</span>
				{/if}
				...
				<div class="fy-card__body">
					...
					<div class="fy-card__actions">
						{if $cardStyle == 'theme'}
							<a href="{$p.url}" class="fy-btn fy-btn--secondary fy-btn--sm">{'Demo'|translate}</a>
							<a href="{$p.url}" class="fy-btn fy-btn--primary fy-btn--sm">{'Detail'|translate}</a>
						{else}
							<a href="{$p.url}" class="fy-btn fy-btn--primary fy-btn--sm">{'Wiew'|translate}</a>
							{if $p.in_stock}
								<button type="button" class="fy-btn fy-btn--secondary fy-btn--sm addtocart" data-id="{$p.id_product}">{'Add To Cart'|translate}</button>
							{else}
...
{else}
<div class="fy-empty">{'Product not found'|translate}</div>
{/if}
