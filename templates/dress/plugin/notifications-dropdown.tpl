{if $isLoggedIn}
<div class="notification-dropdown-wrap" data-empty="{'No notifications yet'|translate}">
	<button type="button" class="notification-bell-btn action-icon" aria-expanded="false" aria-haspopup="true" title="{'Notifications'|translate}">
		...
		<span class="notification-bell-badge{if $notificationCount <= 0} d-none{/if}">{$notificationCount}</span>
	</button>

	<div class="notification-dropdown" hidden>
		<div class="notification-dropdown__head">
			<strong>{'Notifications'|translate}</strong>
			<button type="button" class="btn btn-link btn-sm p-0 notification-mark-all-read{if $notificationCount <= 0} d-none{/if}">{'Mark all read'|translate}</button>
		</div>
		<div class="notification-dropdown__list">
			{if $headerNotifications|@count}
				{foreach $headerNotifications as $n}
				<a href="{$n.url|escape}" class="notification-dropdown__item{if !$n.is_read} is-unread{/if}" data-id="{$n.id_notification}">
				...
				{/foreach}
			{else}
				<div class="notification-dropdown__empty">{'No notifications yet'|translate}</div>
			{/if}
		</div>
