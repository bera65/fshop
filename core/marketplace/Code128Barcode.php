...
class Code128Barcode
{
	...
	public static function toSvg(string $text, int $height = 56, float $module = 1.5): string
	{
		$text = preg_replace('/[^\x20-\x7E]/', '', $text);
		$text = $text !== null && $text !== '' ? $text : '0';
		...
		foreach ($codes as $code) {
			$pattern = $patterns[(int) $code] ?? '211214';
			$isBar = true;

			for ($p = 0, $plen = strlen($pattern); $p < $plen; $p++) {
				$w = ((int) $pattern[$p]) * $module;
