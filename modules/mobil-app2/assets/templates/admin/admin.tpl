...
<form method="post" enctype="multipart/form-data" class="mobil-app-admin">
	...
	<div class="card border-0 shadow-sm mb-4">
		<div class="card-header bg-white">
			<strong>Push bildirimleri (Web Push)</strong>
		</div>
		<div class="card-body">
			<p class="text-muted small">
				Android ve masaüstünde uygulama kapalıyken sistem bildirimi göndermek için Web Push kullanılır.
				Ücretsizdir; ek kurulum gerekmez. HTTPS ve müşteri izni şarttır. iOS’ta yalnızca ana ekrana eklenmiş PWA’da çalışır.
			</p>

			{if !$pushLibraryReady}
			<div class="alert alert-danger small mb-3">
				<strong>Push kütüphanesi eksik.</strong> Sunucuya <code>modules/mobil-app/vendor/</code> klasörünün tam yüklendiğinden emin olun (modül ZIP ile birlikte gelmeli).
			</div>
			{elseif !$pushAvailable}
			<div class="alert alert-warning small mb-3">
				Push aktif değil veya VAPID anahtarları eksik. VAPID oluşturup push’u etkinleştirin.
			</div>
			{/if}

			{if !$pushOpenSslReady}
			<div class="alert alert-warning small mb-3">
				Sunucuda OpenSSL EC veya cURL eksik; push bildirimleri çalışmayabilir.
			</div>
			{/if}

			<div class="form-check form-switch mb-3">
				<input class="form-check-input" type="checkbox" id="push_enabled" name="push_enabled" value="1"{if $settings.push_enabled} checked{/if}{if !$pushOpenSslReady} disabled{/if}>
				<label class="form-check-label" for="push_enabled">Push bildirimleri aktif</label>
			</div>

			<div class="row g-3 mb-3">
				<div class="col-md-6">
					<label class="form-label" for="vapid_subject">VAPID subject (mailto: veya site URL)</label>
					<input type="text" class="form-control" id="vapid_subject" name="vapid_subject" maxlength="255" value="{$settings.vapid_subject|escape}" placeholder="https://magaza.com veya mailto:destek@magaza.com">
				</div>
				<div class="col-md-6">
					<label class="form-label">Kayıtlı cihaz aboneliği</label>
					<div class="form-control-plaintext"><strong>{$pushSubscriptionCount}</strong> cihaz</div>
				</div>
				<div class="col-12">
					<label class="form-label" for="vapid_public">VAPID public key</label>
					<input type="text" class="form-control font-monospace small" id="vapid_public" name="vapid_public" value="{$settings.vapid_public|escape}" readonly>
				</div>
				<div class="col-12">
					<label class="form-label" for="vapid_private">VAPID private key</label>
					<input type="password" class="form-control font-monospace small" id="vapid_private" name="vapid_private" value="{$settings.vapid_private|escape}" autocomplete="new-password" placeholder="Kayıtlı anahtar korunur">
				</div>
			</div>

			<button type="submit" form="mobil-app-generate-vapid" class="btn btn-outline-secondary btn-sm me-2"{if !$pushOpenSslReady} disabled{/if}>
				VAPID anahtarı oluştur
			</button>
			<button type="submit" form="mobil-app-test-push" class="btn btn-outline-primary btn-sm"{if !$pushAvailable || $pushSubscriptionCount <= 0} disabled{/if}>
				Tüm cihazlara test bildirimi gönder
			</button>
			<button type="submit" form="mobil-app-clear-push" class="btn btn-outline-danger btn-sm ms-2"{if $pushSubscriptionCount <= 0} disabled{/if}>
				Abonelikleri sıfırla
			</button>
			<p class="text-muted small mt-2 mb-0">410 Gone = eski/geçersiz abonelik; sıfırlayıp müşterinin tekrar izin vermesini sağlayın. VAPID değişince abonelikler otomatik silinir.</p>
		</div>
	</div>
...
<form id="mobil-app-generate-vapid" method="post" class="d-none">
	<input type="hidden" name="token" value="{$adminToken|escape}">
	<input type="hidden" name="generateMobilAppVapid" value="1">
</form>

<form id="mobil-app-test-push" method="post" class="d-none">
	<input type="hidden" name="token" value="{$adminToken|escape}">
	<input type="hidden" name="testMobilAppPush" value="1">
</form>

<form id="mobil-app-clear-push" method="post" class="d-none">
	<input type="hidden" name="token" value="{$adminToken|escape}">
	<input type="hidden" name="clearMobilAppPushSubs" value="1">
</form>
