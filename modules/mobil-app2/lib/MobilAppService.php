...
class MobilAppService
{
	...
	public static function ensureSchema(): void
	{
		...
		self::ensurePushSchema();
	}

	public static function ensurePushSchema(): void
	{
		require_once __DIR__ . '/PushSubscriptionService.php';
		PushSubscriptionService::ensureSchema();

		$columns = DB::execute('SHOW COLUMNS FROM `' . self::TABLE . '`') ?: [];
		$existing = [];
		...
		$additions = [
			'push_enabled' => "ADD COLUMN `push_enabled` tinyint(1) NOT NULL DEFAULT 0 AFTER `enabled`",
			'vapid_public' => "ADD COLUMN `vapid_public` varchar(255) NOT NULL DEFAULT '' AFTER `offline_message`",
			'vapid_private' => "ADD COLUMN `vapid_private` varchar(255) NOT NULL DEFAULT '' AFTER `vapid_public`",
			'vapid_subject' => "ADD COLUMN `vapid_subject` varchar(255) NOT NULL DEFAULT '' AFTER `vapid_private`",
		];

		foreach ($additions as $name => $sql) {
			if (empty($existing[$name])) {
				DB::execute('ALTER TABLE `' . self::TABLE . '` ' . $sql);
			}
		}
	}

	public static function ensurePushKeys(): void
	{
		...
		require_once dirname(__FILE__) . '/WebPushNative.php';
		$generated = WebPushNative::createVapidKeys();

		if (($generated['publicKey'] ?? '') === '' || ($generated['privateKey'] ?? '') === '') {
			return;
		}

		self::saveVapidKeys($generated['publicKey'], $generated['privateKey'], true);
	}

	public static function ensurePushFooterHook(): void
	{
		if (!self::isModuleActive()) {
			return;
		}
		...
		$existing = DB::execute(
			"SELECT id_hook FROM module_display_hooks WHERE module_name = 'mobil-app' AND hook_name = 'footer' LIMIT 1"
		);
		...
		$maxPos = (int) DB::getValue(
			"SELECT COALESCE(MAX(position), 0) FROM module_display_hooks WHERE hook_name = 'footer'"
		);

		DB::insert('module_display_hooks', [
			'module_name' => 'mobil-app',
			'hook_name' => 'footer',
			'position' => $maxPos + 1,
		]);
	}
	...
	public static function isPushEnabled(): bool
	{
		...
		return $row && (int) ($row['push_enabled'] ?? 0) === 1;
	}
	...
	public static function getVapidKeys(): array
	{
		...
		return [
			'public' => trim((string) ($row['vapid_public'] ?? '')),
			'private' => trim((string) ($row['vapid_private'] ?? '')),
			'subject' => trim((string) ($row['vapid_subject'] ?? '')),
		];
	}

	public static function getVapidSubject(): string
	{
		$keys = self::getVapidKeys();
	...
	public static function saveSettings(array $input): bool
	{
		...
		$data = [
			...
			'vapid_public' => self::clip((string) ($input['vapid_public'] ?? ''), 255),
			'vapid_private' => self::clip((string) ($input['vapid_private'] ?? ''), 255),
			'vapid_subject' => self::clip((string) ($input['vapid_subject'] ?? ''), 255),
		];
		...
		if (trim((string) ($data['vapid_public'] ?? '')) === '') {
			$data['vapid_public'] = (string) ($current['vapid_public'] ?? '');
		}

		if (trim((string) ($data['vapid_private'] ?? '')) === '') {
			$data['vapid_private'] = (string) ($current['vapid_private'] ?? '');
		}

		if (trim((string) ($data['vapid_subject'] ?? '')) === '') {
			$data['vapid_subject'] = (string) ($current['vapid_subject'] ?? '');
		}

		if (
			trim((string) ($data['vapid_public'] ?? '')) !== ''
			&& trim((string) ($data['vapid_public'] ?? '')) !== trim((string) ($current['vapid_public'] ?? ''))
		) {
			self::clearPushSubscriptions();
		}
	...
	public static function saveVapidKeys(string $public, string $private, bool $enablePush = true): bool
	{
		self::ensureSchema();
		$current = self::getSettings();

		if ($public !== '' && $public !== (string) ($current['vapid_public'] ?? '')) {
			self::clearPushSubscriptions();
		}

		return self::saveSettings(array_merge($current, [
			'vapid_public' => $public,
			'vapid_private' => $private,
			'push_enabled' => $enablePush ? 1 : 0,
		]));
	}

	public static function clearPushSubscriptions(): void
	{
		require_once __DIR__ . '/PushSubscriptionService.php';
		PushSubscriptionService::ensureSchema();
		DB::execute('DELETE FROM mobil_app_push_subscriptions');
	}
	...
	/** Site köküne göre göreli public yol (CSP / origin uyumu için). */
	public static function publicPath(string $path = ''): string
	{
		$scope = rtrim(self::getScopePath(), '/');
		$path = ltrim($path, '/');
...
self.addEventListener('push', (event) => {
  let payload = { title: 'Bildirim', body: '', url: SCOPE_PATH, icon: DEFAULT_ICON };
  ...
  const targetUrl = payload.url || SCOPE_PATH;
  event.waitUntil(
...
JS;
	...
	private static function normalizeRow(array $row): array
	{
		return [
			...
			'push_enabled' => (int) ($row['push_enabled'] ?? 0),
			'vapid_public' => (string) ($row['vapid_public'] ?? ''),
			'vapid_private' => (string) ($row['vapid_private'] ?? ''),
			'vapid_subject' => (string) ($row['vapid_subject'] ?? ''),
		];
	}
